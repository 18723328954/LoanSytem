...
export default {
  ...
  methods: {
    async getLineData() {
      let resData = {}
      var data = (await getDepartmentLineData(this.$store.getters.department_id)).data
      var lineLabel = []
      var lineData = []
    ...
    async getCallBarData() {
      let data = (await getDepartmentCallBarData(this.$store.getters.department_id)).data

      return data
    },
    // 意向客户柱状图
    async getCustomerData() {
      let data = (await getDepartmentCustomerBarData(this.$store.getters.department_id)).data
